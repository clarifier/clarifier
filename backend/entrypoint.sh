...
if [ -t 0 ]; then
	...
	/usr/src/backend/manage.py wait_for_db
	if ! /usr/src/backend/manage.py migrate --check >/dev/null 2>&1; then
		echo "💾 Applying database migrations"
		/usr/src/backend/manage.py migrate --no-input
	fi
	if [ "$DJANGO_SUPERUSER_USERNAME" ]; then
		echo "👤 Ensuring admin user"
		/usr/src/backend/manage.py create_admin \
			--noinput \
